...
test("creation form resets the destination/departure selection after a successful submit, and closeCreateModal (× / Cancel / Escape) does the same after saving a draft", () => {
  ...
  assert.match(page, /setModalOpen\(false\);\s*setEditingDeliveryId\(null\);\s*setEditingOriginal\(null\);\s*setParcelDrafts\(\[emptyParcelDraft\("0"\)\]\);\s*setCreationDestinationSiteId\(""\);\s*setCreationDepartureAt\(""\);\s*setCreationDraftSeed\(null\);/);
