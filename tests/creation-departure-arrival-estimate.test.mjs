...
test("creation form resets the destination/departure selection after a successful submit, and closeCreateModal (× / Cancel / Escape) does the same after saving a draft", () => {
  ...
  assert.match(page, /setModalOpen\(false\);\s*setEditingDeliveryId\(null\);\s*setEditingOriginal\(null\);\s*setParcelDrafts\(\[\{ key: "0", weightKg: "", manualPriceAmount: "", itemDescription: "", paymentStatus: "unpaid", amountPaid: "" \}\]\);\s*setSharedWeightTotal\(""\);\s*setCreationDestinationSiteId\(""\);\s*setCreationDepartureAt\(""\);\s*setCreationDraftSeed\(null\);/);
