...
test("parses a quoted delivery CSV row", () => {
  const result = parseBulkDeliveryCsv([
    "customer,destination,planned_arrival_at,origin_site_id,truck,contact,whatsapp_opt_in",
    '"Client, SARL","Casablanca, Maroc",2026-08-20T14:00:00+02:00,brussels-abattoir-45,TRUCK-01,+212600000000,oui',
  ].join("\n"));
...
test("reports invalid required fields before import", () => {
  const result = parseBulkDeliveryCsv([
    "customer,destination,planned_arrival_at,origin_site_id,truck",
    ",Casablanca,nope,brussels-abattoir-45,",
  ].join("\n"));
...
test("origin_site_id is required, both as a header and a per-row value -- price depends on it (EUR vs MAD)", () => {
  const missingHeader = parseBulkDeliveryCsv([
    ...
    "Client sans origine,Casablanca,2026-08-20T14:00:00Z",
  ].join("\n"));
...
test("accepts a bulk parcel without a truck and queues it for assignment", () => {
  const result = parseBulkDeliveryCsv([
    "customer,destination,planned_arrival_at,origin_site_id",
    "Client sans camion,Casablanca,2026-08-20T14:00:00Z,brussels-abattoir-45",
  ].join("\n"));
  assert.deepEqual(result.errors, []);
...
test("parses optional weight without inventing missing values", () => {
  const result = parseBulkDeliveryCsv([
    "customer,destination,planned_arrival_at,origin_site_id,truck,weight_kg",
    "Client EUR,Casa,2026-08-20T14:00:00Z,brussels-abattoir-45,T1,12.3456",
    "Client MAD,Tanger,2026-08-21T14:00:00Z,brussels-abattoir-45,T2,",
  ].join("\n"));
  assert.deepEqual(result.errors, []);
...
test("rejects invalid weight values", () => {
  const result = parseBulkDeliveryCsv([
    "customer,destination,planned_arrival_at,origin_site_id,truck,weight_kg",
    "A,Casa,2026-08-20T14:00:00Z,brussels-abattoir-45,T1,-1",
  ].join("\n"));
  assert.equal(result.rows.length, 0);
...
test("rejects price columns -- price is always computed from weight and origin, never imported", () => {
  const result = parseBulkDeliveryCsv([
    "customer,destination,planned_arrival_at,origin_site_id,truck,price_amount",
    "A,Casa,2026-08-20T14:00:00Z,brussels-abattoir-45,T1,45",
  ].join("\n"));
  assert.equal(result.rows.length, 0);
...
test("rejects unknown headers rather than silently dropping data", () => {
  const result = parseBulkDeliveryCsv([
    "customer,destination,planned_arrival_at,origin_site_id,truck,secret_note",
    "A,Casa,2026-08-20T14:00:00Z,brussels-abattoir-45,T1,hello",
  ].join("\n"));
  assert.equal(result.rows.length, 0);
...
test("caps a single import to the operational safety limit", () => {
  const rows = Array.from({ length: MAX_BULK_DELIVERY_ROWS + 1 }, (_, index) => `C${index},Casa,2026-08-20T14:00:00Z,brussels-abattoir-45,T1`);
  const result = parseBulkDeliveryCsv(["customer,destination,planned_arrival_at,origin_site_id,truck", ...rows].join("\n"));
  assert.equal(result.rows.length, 0);
