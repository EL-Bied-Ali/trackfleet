...
test("automation records every live vehicle using canonical physical identity before delivery-specific processing", () => {
  ...
  assert.match(automation, /canonicalFleetVehicleId\(vehicle\.name, vehicle\.providerDeviceId \|\| vehicle\.id\)/);
  assert.match(automation, /canonicalFleetVehicleId\(delivery\.truck, delivery\.sendatrackVehicleId\)/);
