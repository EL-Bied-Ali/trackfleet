...
export default function SiteManager({ locale }: { locale: "fr" | "en" | "nl" }) {
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      shortCodePrefix: String(form.get("shortCodePrefix") ?? "").trim().toUpperCase() || null,
    };
    try {
    ...
    {open && <div className="modal-backdrop">
      <section className="modal site-manager-modal" role="dialog" aria-modal="true" aria-labelledby="sites-title">
        ...
        {siteFormOpen && <form className="site-manager-form" key={editingSite?.id ?? "new-site"} onSubmit={save}>
          ...
          <label>{copy.shortCodePrefix}<input name="shortCodePrefix" style={{ textTransform: "uppercase" }} maxLength={12} placeholder="CAS" defaultValue={editingSite?.shortCodePrefix ?? ""} /></label>
