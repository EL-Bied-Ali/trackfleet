...
export default function SiteManager({ locale }: { locale: "fr" | "en" | "nl" }) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 409 && data?.error === "arrival_blocked_missing_scans") {
          setCompletionError("arrival_blocked_missing_scans");
          setArrivalBlockedScans({ missingLoaded: Boolean(data.missingLoadedScan), missingHub: Boolean(data.missingHubScan) });
          return;
        }
        throw new Error("arrival_failed");
      }
      setManualDeliveries((items) => items.map((item) => item.id === delivery.id
        ...
        : item));
      // Reported live: this panel confirmed arrival but never told the
      // customer, unlike the delivery table's group-level arrival buttons
      // (same underlying manual-completion action) which always attempt the
      // free WhatsApp notice. Same best-effort call, same honest reporting
      // of whether it actually went out.
      const notifyResponse = await fetch("/api/deliveries/notify-arrival", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ deliveryId: delivery.id }),
      }).then((res) => res.json() as Promise<{ ok?: boolean }>).catch(() => ({ ok: false }));
      setCompletionNotice(notifyResponse.ok ? copy.arrivalNotified : copy.arrivalNotNotified);
    } catch {
  ...
  async function confirmDeparture(delivery: ManualDelivery) {
    ...
    try {
      ...
      // Same gap as confirmArrival above: this panel changed the status but
      // never told the customer, unlike the delivery table's group-level
      // departure button (same underlying manual-completion action).
      const notifyResponse = await fetch("/api/deliveries/notify-departure", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ deliveryId: delivery.id }),
      }).then((res) => res.json() as Promise<{ ok?: boolean }>).catch(() => ({ ok: false }));
      setCompletionNotice(notifyResponse.ok ? copy.departureNotified : copy.departureNotNotified);
    } catch {
  ...
  const copy = locale === "fr"
    ...
    : locale === "nl"
      ...
      : {
          ...
          arrivalNotified: "Arrival confirmed, customer notified via WhatsApp", arrivalNotNotified: "Arrival confirmed (WhatsApp not sent: 24h window closed or consent withdrawn)", departureNotified: "Departure confirmed, customer notified via WhatsApp", departureNotNotified: "Departure confirmed (WhatsApp not sent: 24h window closed or consent withdrawn)",
        };
  ...
  return <>
    ...
    {open && <div className="modal-backdrop">
      <section className="modal site-manager-modal" role="dialog" aria-modal="true" aria-labelledby="sites-title">
        ...
        <div className="site-manager-list">
          {visibleSites.map((site) => {
            ...
            return <article key={site.id} className="site-manager-card">
              <div><span className={`site-gps-status ${gpsReady ? "ready" : "missing"}`}>{gpsReady ? copy.gpsReady : copy.gpsMissing}</span><strong><i aria-hidden="true" style={{ display: "inline-block", width: 10, height: 10, marginRight: 6, borderRadius: "50%", background: site.color, border: "1px solid rgba(0,0,0,.15)" }} />{site.label}</strong><p>{site.address}</p>{site.whatsapp && <p>{site.whatsapp}</p>}</div>
        ...
        {siteFormOpen && <form className="site-manager-form" key={editingSite?.id ?? "new-site"} onSubmit={save}>
          ...
          <div className="form-row"><label>{copy.whatsapp}<input name="whatsapp" type="tel" defaultValue={editingSite?.whatsapp ?? ""} /></label><label>{copy.color}<input name="color" type="color" defaultValue={editingSite?.color ?? defaultSiteColor} style={{ height: 44, padding: 4 }} /></label></div>
    ...
    {completionOpen && <div className="modal-backdrop">
      <section className="modal" role="dialog" aria-modal="true" aria-labelledby="completion-title">
            ...
            const arrivalLabel = unassigned
              ...
              : delivery.arrivalState === "automatic_confirmed"
                ...
                : delivery.arrivalState === "manual_confirmed"
                  ...
                  : delivery.arrivalReason === "ctm_relay_in_progress"
                    ? copy.relayInProgress
        ...
        {completionError && <p className="login-error">{completionError === "arrival_blocked_missing_scans" && arrivalBlockedScans ? copy.arrivalBlocked(arrivalBlockedScans.missingLoaded, arrivalBlockedScans.missingHub) : copy.completionError}</p>}
        {completionNotice && <p className="agency-location-message" role="status">{completionNotice}</p>}
        <div className="modal-footer"><button type="button" onClick={() => { setCompletionOpen(false); setCompletionNotice(""); }}>{copy.close}</button></div>
      </section>
    </div>}
  </>;
}
