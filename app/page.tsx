...
// Same palette as the truck badges, keyed by a stable hash of the raw
// destination address instead of an assigned index -- a group's arrival
// checkmarks and each row's own destination cell need to agree on the exact
// same color for the exact same string with no shared counter between them,
// so a lookup table (which would need one) isn't an option here.
function destinationColor(destination: string) {
  let hash = 0;
  for (let index = 0; index < destination.length; index += 1) hash = (hash * 31 + destination.charCodeAt(index)) | 0;
  return truckBadgeColors[Math.abs(hash) % truckBadgeColors.length];
}
