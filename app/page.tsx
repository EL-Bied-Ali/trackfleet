...
export default function Home() {
  ...
  const [parcelDrafts, setParcelDrafts] = useState<Array<{ key: string; weightKg: string; manualPriceAmount: string; itemDescription: string; paymentStatus: "unpaid" | "partial" | "paid"; amountPaid: string }>>([{ key: "0", weightKg: "", manualPriceAmount: "", itemDescription: "", paymentStatus: "unpaid", amountPaid: "" }]);
  // The depot scale weighs several parcels for the same client at once (a
  // real workflow the client won't change) -- rather than force the
  // employee to weigh each one separately, this splits one combined weight
  // evenly across the parcel rows already added above. Capped at 5 (the
  // client's own stated max for one weighing) purely as a UI affordance;
  // nothing downstream depends on that number.
  const [sharedWeightTotal, setSharedWeightTotal] = useState("");
  ...
  const closeCreateModal = useCallback(() => {
    ...
    setParcelDrafts([{ key: "0", weightKg: "", manualPriceAmount: "", itemDescription: "", paymentStatus: "unpaid", amountPaid: "" }]);
    setSharedWeightTotal("");
  ...
  function openCreateModal() {
    ...
    if (draft) {
      ...
      setParcelDrafts(draft.parcels.length ? draft.parcels.map((parcel) => ({ ...parcel, paymentStatus: parcel.paymentStatus ?? "unpaid", amountPaid: parcel.amountPaid ?? "" })) : [{ key: "0", weightKg: "", manualPriceAmount: "", itemDescription: "", paymentStatus: "unpaid", amountPaid: "" }]);
