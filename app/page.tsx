...
export default function Home() {
  ...
  const [truckEditorDeliveryId, setTruckEditorDeliveryId] = useState<string | null>(null);
  const [truckEditorSelection, setTruckEditorSelection] = useState("");
  const [truckEditorPending, setTruckEditorPending] = useState(false);
  const [scheduleEditorDeliveryId, setScheduleEditorDeliveryId] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (!truckEditorDeliveryId) return;
    const close = (event: MouseEvent) => {
      const target = event.target as HTMLElement | null;
      if (target?.closest(".truck-editor-popover, .truck-editor-trigger")) return;
      setTruckEditorDeliveryId(null);
    };
    document.addEventListener("click", close);
    return () => document.removeEventListener("click", close);
  }, [truckEditorDeliveryId]);

  useEffect(() => {
    if (!scheduleEditorDeliveryId) return;
    const close = (event: MouseEvent) => {
      const target = event.target as HTMLElement | null;
      if (target?.closest(".schedule-editor-popover, .schedule-editor-trigger")) return;
      setScheduleEditorDeliveryId(null);
    };
    document.addEventListener("click", close);
    return () => document.removeEventListener("click", close);
  }, [scheduleEditorDeliveryId]);
