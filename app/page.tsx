...
import {
  ...
  type DeliveryCreationDraft, type DeliveryCreationDraftParcel,
...
function emptyParcelDraft(key: string): DeliveryCreationDraftParcel {
  return { key, weightKg: "", manualPriceAmount: "", itemDescription: "", paymentStatus: "unpaid", amountPaid: "", groupedWithPrevious: false };
}
...
// to weigh each one separately, checking "compté avec le colis précédent"
// on a row folds it into a run of consecutive grouped rows. The run's first
// (un-grouped) row is the anchor: its own weightKg input is read as the
// TOTAL for the whole run, split evenly across every row in it (anchor
// included). A standalone row (groupSize 1) is unaffected -- its own
// weightKg passes through unchanged, same as before this existed.
function parcelWeightGroups(drafts: DeliveryCreationDraftParcel[]): Array<{ effectiveWeightKg: string; groupSize: number; isAnchor: boolean }> {
  const groups: Array<{ effectiveWeightKg: string; groupSize: number; isAnchor: boolean }> = [];
  let i = 0;
  while (i < drafts.length) {
    let j = i;
    while (j + 1 < drafts.length && drafts[j + 1].groupedWithPrevious) j++;
    const groupSize = j - i + 1;
    const totalRaw = drafts[i].weightKg.trim();
    const total = totalRaw ? Number(totalRaw) : null;
    const each = groupSize > 1 && total !== null && Number.isFinite(total) && total > 0
      ? String(Math.round((total / groupSize) * 1000) / 1000)
      : drafts[i].weightKg;
    for (let k = i; k <= j; k++) groups.push({ effectiveWeightKg: groupSize > 1 ? each : drafts[k].weightKg, groupSize, isAnchor: k === i });
    i = j + 1;
  }
  return groups;
}
...
function LoginScreen({ locale, busy, error, onLocale, onSubmit, googleLink, googleLinkBusy, googleLinkError, googleError, onGoogleLinkSubmit, onGoogleLinkCancel }: {
  ...
  return <main className="login-page">
    <header className="login-header"><span className="brand brand-dark"><span className="brand-mark"><span>↗</span></span><span>TrackFleet</span></span><div style={{ display: "flex", alignItems: "center", gap: 18 }}><a href={`/pricing?lang=${locale}`} style={{ color: "#58424d", fontSize: 12, fontWeight: 700, textDecoration: "none" }}>{copy.pricingLink}</a><LanguageSwitcher locale={locale} label="Language" onChange={onLocale} /></div></header>
...
export default function Home() {
  ...
  const [parcelDrafts, setParcelDrafts] = useState<DeliveryCreationDraftParcel[]>([emptyParcelDraft("0")]);
  const weightGroups = useMemo(() => parcelWeightGroups(parcelDrafts), [parcelDrafts]);
  ...
  const closeCreateModal = useCallback(() => {
    ...
    setParcelDrafts([emptyParcelDraft("0")]);
  ...
  useEffect(() => {
    ...
    const close = (event: MouseEvent) => {
      ...
      if (target?.closest(".contact-popover, .contact-trigger, .scan-location-trigger")) return;
  ...
  const scanAtLabel = (value?: string | null) => value
    ...
    : "—";
  // Known site labels and reverse-geocoded scan labels both use the same
  // "City · detail" convention (see known-sites.ts and app/api/scan/route.ts's
  // reverseGeocodedLabel) -- splitting on the first " · " lets one render
  // path show the city compactly and reveal the rest only on click, per live
  // feedback: "je voulais plutot le nom de la ville et si possible un
  // quartier aproximatif quand on clique" (not raw coordinates, not a tooltip).
  const splitScanLocation = (label?: string | null) => {
    if (!label) return null;
    const separatorIndex = label.indexOf(" · ");
    return separatorIndex === -1
      ? { city: label, detail: null as string | null }
      : { city: label.slice(0, separatorIndex), detail: label.slice(separatorIndex + 3) };
  };
  const renderScanLocation = (deliveryId: string, field: "loadedLocation" | "hubLocation", label?: string | null) => {
    const location = splitScanLocation(label);
    if (!location) return null;
    const popoverKey = `${deliveryId}:${field}`;
    return <>
      <button type="button" className="scan-location-trigger" onClick={(event) => { event.stopPropagation(); setOpenContactPopover((current) => current === popoverKey ? null : popoverKey); }}>· {location.city}</button>
      {openContactPopover === popoverKey && <div className="contact-popover scan-location-popover"><strong>{locale === "fr" ? "Lieu du scan" : locale === "nl" ? "Scanlocatie" : "Scan location"}</strong><span>{location.detail ? `${location.city} · ${location.detail}` : location.city}</span></div>}
    </>;
  };
  ...
  // Assigned by index over every distinct destination currently on screen
  // (sorted, for a stable order run to run), not hashed independently per
  // string -- a handful of destinations sharing a 10-color palette collides
  // far more often than it looks like it should (5 items into 10 slots is
  ...
  function openCreateModal() {
    ...
    // default silently carried forward from the previous delivery. Scoped
    // to "In transit" only -- a truck still "Loading" at origin is exactly
    // the normal multi-parcel-per-truck workflow and should keep defaulting
    // to it. Kept even though creation no longer captures a GPS baseline
    // immediately (see the delivery-store applySendatrackSnapshot/linkVehicle
    // changes) -- a truck already out still shouldn't be the silent default
    // for a new, unrelated delivery.
    ...
    if (draft) {
      ...
      // A draft saved before payment tracking existed won't carry these two
      // fields at all -- default them rather than leaving `undefined`
      // sitting in state (which would break the pill selector below).
      setParcelDrafts(draft.parcels.length ? draft.parcels.map((parcel) => ({ ...parcel, paymentStatus: parcel.paymentStatus ?? "unpaid", amountPaid: parcel.amountPaid ?? "", groupedWithPrevious: parcel.groupedWithPrevious ?? false })) : [emptyParcelDraft("0")]);
  ...
  function openEditModal(delivery: Delivery) {
    ...
    setParcelDrafts([{
      ...
      // Pre-filled with the delivery's current price (whether it was
      // originally weight-computed or already manually overridden), not
      // left blank -- an edit to an unrelated field (e.g. the customer's
      // phone number) must never silently reset an already-agreed price
      // back to the weight formula.
      manualPriceAmount: delivery.priceAmount != null ? String(delivery.priceAmount) : "",
      itemDescription: delivery.itemDescription ?? "",
      paymentStatus: delivery.paymentStatus ?? "unpaid",
      amountPaid: delivery.amountPaid != null ? String(delivery.amountPaid) : "",
  ...
  async function saveDeliveryEdits(event: React.FormEvent<HTMLFormElement>, deliveryId: string) {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    const parcel = parcelDrafts[0] ?? { weightKg: "", manualPriceAmount: "", itemDescription: "", paymentStatus: "unpaid" as const, amountPaid: "" };
    setCreating(true);
    try {
      const response = await fetch("/api/deliveries/update", {
        ...
        body: JSON.stringify({
          ...
          amountPaid: parcel.paymentStatus === "partial" && parcel.amountPaid.trim() ? Number(parcel.amountPaid) : null,
        }),
      });
  ...
  async function createDelivery(event: React.FormEvent<HTMLFormElement>) {
    ...
    event.preventDefault();
    if (weightGroups.some((group) => group.groupSize > 1 && !group.effectiveWeightKg)) {
      setToast(locale === "fr" ? "Indiquez le poids total pour les colis groupés avant de créer la livraison." : locale === "nl" ? "Geef het totaalgewicht op voor de gegroepeerde pakketten voor u de zending aanmaakt." : "Enter the total weight for the grouped parcels before creating the delivery.");
      return;
    }
    const form = new FormData(event.currentTarget);
    ...
    let destinationSiteId = String(form.get("destinationSiteId") ?? "").trim();
    let selectedSite = knownSites.find((site) => site.id === destinationSiteId);
    if (destinationSiteId === "__new_agency__") {
      const newAgencyLabel = String(form.get("newAgencyLabel") ?? "").trim();
      const newAgencyCity = String(form.get("newAgencyCity") ?? "").trim();
      const newAgencyAddress = String(form.get("newAgencyAddress") ?? "").trim();
      ...
      try {
        ...
        if (!response.ok || !data.site) {
          setToast(locale === "fr" ? "Impossible de créer cette nouvelle agence." : locale === "nl" ? "Kon dit nieuwe agentschap niet aanmaken." : "Could not create this new agency.");
          setCreating(false);
          return;
        }
        // So it's immediately usable as this delivery's destination below,
        // and shows up like any other agency next time the form/select
        // opens -- same event SiteManager's own save() dispatches.
        setKnownSites((sites) => [...sites, data.site!]);
        window.dispatchEvent(new Event("trackfleet-sites-changed"));
        selectedSite = data.site;
        destinationSiteId = data.site.id;
      } catch {
        setToast(locale === "fr" ? "Impossible de créer cette nouvelle agence." : locale === "nl" ? "Kon dit nieuwe agentschap niet aanmaken." : "Could not create this new agency.");
        setCreating(false);
        return;
    ...
    for (let parcelIndex = 0; parcelIndex < parcelDrafts.length; parcelIndex++) {
      const parcel = parcelDrafts[parcelIndex];
      const weightRaw = weightGroups[parcelIndex].effectiveWeightKg.trim();
      const manualPriceRaw = parcel.manualPriceAmount.trim();
      const amountPaidRaw = parcel.amountPaid.trim();
      const draftDelivery = {
        ...sharedFields,
        shipmentId,
        weightKg: weightRaw ? Number(weightRaw) : null,
        manualPriceAmount: manualPriceRaw ? Number(manualPriceRaw) : null,
        itemDescription: weightRaw ? "" : parcel.itemDescription.trim(),
        paymentStatus: parcel.paymentStatus,
        amountPaid: parcel.paymentStatus === "partial" && amountPaidRaw ? Number(amountPaidRaw) : null,
      };
      try {
  ...
  return (
    <main className="app-shell">
      ...
      {modalOpen && <div className="modal-backdrop"><section className="modal delivery-modal" role="dialog" aria-modal="true" aria-labelledby="new-delivery-title"><div className="modal-header"><div><p className="eyebrow">{editingDeliveryId ? (locale === "fr" ? "MODIFIER" : locale === "nl" ? "BEWERKEN" : "EDIT") : t.createEyebrow}</p><h2 id="new-delivery-title">{editingDeliveryId ? (locale === "fr" ? "Modifier la livraison" : locale === "nl" ? "Zending bewerken" : "Edit delivery") : t.createTitle}</h2><span>{editingDeliveryId ? (locale === "fr" ? "Le camion et la date de départ utilisent leurs propres confirmations, séparées de l’enregistrement du reste." : locale === "nl" ? "Voertuig en vertrekdatum gebruiken hun eigen bevestiging, los van het opslaan van de rest." : "Truck and departure date are confirmed separately from saving the rest.") : (integration.connected ? t.createHelpAutomatic : t.createHelp)}</span></div><button onClick={closeCreateModal} aria-label={t.close}>×</button></div><form onSubmit={createDelivery} ref={creationFormRef}><div className="form-row"><label>{locale === "fr" ? "Site de départ" : locale === "nl" ? "Vertreklocatie" : "Origin site"}<select name="originSiteId" required value={defaultOriginSiteId} disabled={company?.role === "agency" || Boolean(editingDeliveryId)} onChange={(event) => { const siteId = event.target.value; setDefaultOriginSiteId(siteId); if (company) window.localStorage.setItem(originPreferenceKey(company), siteId); }}><option value="" disabled>{locale === "fr" ? "Choisir le site" : locale === "nl" ? "Kies locatie" : "Choose site"}</option>{knownSites.filter((site) => site.roles.includes("origin") && (company?.role !== "agency" || site.id === company.siteId)).map((site) => <option key={site.id} value={site.id}>{site.label}</option>)}</select><small>{company?.role === "agency" ? (locale === "fr" ? "Les colis enregistrés sont automatiquement rattachés à votre agence." : locale === "nl" ? "Geregistreerde zendingen worden automatisch aan uw agentschap gekoppeld." : "Registered parcels are automatically assigned to your agency.") : (locale === "fr" ? "Ce choix sera mémorisé pour cet utilisateur sur ce navigateur." : locale === "nl" ? "Deze keuze wordt voor deze gebruiker in deze browser onthouden." : "This choice will be remembered for this user on this browser.")}</small></label><label>{t.destination}<select name="destinationSiteId" required value={creationDestinationSiteId} onChange={(event) => setCreationDestinationSiteId(event.target.value)}><option value="" disabled>{locale === "fr" ? "Choisir l'agence" : locale === "nl" ? "Kies agentschap" : "Choose agency"}</option>{knownSites.filter((site) => site.roles.includes("destination") && (company?.role !== "agency" || site.country !== creationOriginCountry)).map((site) => <option key={site.id} value={site.id}>{site.label}</option>)}{company?.role !== "agency" && <option value="__new_agency__">{locale === "fr" ? "Autre (nouvelle agence…)" : locale === "nl" ? "Andere (nieuw agentschap…)" : "Other (new agency…)"}</option>}</select></label></div>{creationDestinationSiteId === "__new_agency__" && <div className="form-section"><strong>{locale === "fr" ? "Nouvelle agence" : locale === "nl" ? "Nieuw agentschap" : "New agency"}</strong><div className="form-row"><label>{locale === "fr" ? "Nom de l'agence" : locale === "nl" ? "Naam agentschap" : "Agency name"}<input name="newAgencyLabel" required placeholder={locale === "fr" ? "Ex. : Oujda · Centre-ville" : locale === "nl" ? "Bijv. Oujda · Centrum" : "E.g. Oujda · City centre"} /></label><label>{locale === "fr" ? "Ville" : locale === "nl" ? "Stad" : "City"}<input name="newAgencyCity" required /></label></div><label>{locale === "fr" ? "Adresse" : locale === "nl" ? "Adres" : "Address"}<input name="newAgencyAddress" required /></label><label><span className="field-label">{locale === "fr" ? "WhatsApp de l'agence" : locale === "nl" ? "WhatsApp van het agentschap" : "Agency WhatsApp"} <span className="optional-badge">{t.optional}</span></span><input name="newAgencyWhatsapp" type="tel" placeholder="+212…" /></label><small>{locale === "fr" ? "Cette agence sera enregistrée et disponible pour les prochaines livraisons." : locale === "nl" ? "Dit agentschap wordt opgeslagen en is beschikbaar voor volgende zendingen." : "This agency will be saved and available for future deliveries."}</small></div>}<div className="form-section-row"><div className="form-section"><strong>{locale === "fr" ? "Expéditeur / client" : locale === "nl" ? "Afzender / klant" : "Sender / customer"}</strong><div className="form-row"><label>{t.customerCompany}<input name="customer" required placeholder={t.customerPlaceholder} defaultValue={creationDraftSeed?.customer ?? ""} /></label><label><span className="field-label">{t.customerContact} <span className="optional-badge">{t.optional}</span></span><input name="contact" inputMode="tel" autoComplete="tel" placeholder="+32… / +212…" defaultValue={creationDraftSeed?.contact ?? ""} /></label></div><div className="form-row"><label><span className="field-label">{locale === "fr" ? "E-mail du client" : locale === "nl" ? "E-mail klant" : "Customer email"} <span className="optional-badge">{t.optional}</span></span><input name="customerEmail" type="email" autoComplete="email" placeholder="client@exemple.com" defaultValue={creationDraftSeed?.customerEmail ?? ""} /></label></div><small>{locale === "fr" ? "L'e-mail reçoit les mêmes mises à jour de suivi, sans consentement séparé à cocher." : locale === "nl" ? "Dit e-mailadres ontvangt dezelfde trackingupdates, zonder aparte toestemming aan te vinken." : "This email receives the same tracking updates, no separate consent checkbox needed."}</small></div><div className="form-section"><strong>{locale === "fr" ? "Personne qui reçoit le colis" : locale === "nl" ? "Ontvanger van het pakket" : "Parcel recipient"}</strong><div className="form-row"><label><span className="field-label">{locale === "fr" ? "Nom du destinataire" : locale === "nl" ? "Naam ontvanger" : "Recipient name"} <span className="optional-badge">{t.optional}</span></span><input name="recipientName" autoComplete="name" placeholder={locale === "fr" ? "Nom et prénom" : locale === "nl" ? "Voor- en achternaam" : "Full name"} defaultValue={creationDraftSeed?.recipientName ?? ""} /></label><label><span className="field-label">{locale === "fr" ? "Téléphone du destinataire" : locale === "nl" ? "Telefoon ontvanger" : "Recipient phone"} <span className="optional-badge">{t.optional}</span></span><input name="recipientContact" inputMode="tel" autoComplete="tel" placeholder="+32… / +212…" defaultValue={creationDraftSeed?.recipientContact ?? ""} /></label></div><small>{locale === "fr" ? "Renseignez le nom et le téléphone ensemble. Le destinataire recevra les mêmes mises à jour utiles." : locale === "nl" ? "Vul naam en telefoon samen in. De ontvanger krijgt dezelfde nuttige updates." : "Enter name and phone together. The recipient receives the same useful updates."}</small></div></div><div className="parcel-list">{parcelDrafts.map((parcel, index) => { const { effectiveWeightKg, groupSize, isAnchor } = weightGroups[index]; const preview = creationPricePreviewFor(effectiveWeightKg); const manualPriceNumber = Number(parcel.manualPriceAmount); const effectivePriceAmount = manualPriceNumber > 0 ? manualPriceNumber : preview.priceAmount; return <div className="form-row parcel-row" key={parcel.key}>{parcelDrafts.length > 1 && <div className="parcel-row-head">{locale === "fr" ? `Colis ${index + 1}` : locale === "nl" ? `Pakket ${index + 1}` : `Parcel ${index + 1}`}</div>}{index > 0 && <label className="grouped-weight-checkbox"><input type="checkbox" checked={parcel.groupedWithPrevious} onChange={(event) => { const checked = event.target.checked; setParcelDrafts((rows) => rows.map((row, rowIndex) => rowIndex === index ? { ...row, groupedWithPrevious: checked } : row)); }} /><span>{locale === "fr" ? "Compté avec le colis précédent (pesé ensemble)" : locale === "nl" ? "Meegeteld met het vorige pakket (samen gewogen)" : "Counted with the previous parcel (weighed together)"}</span></label>}{isAnchor ? <label><span className="field-label">{groupSize > 1 ? (locale === "fr" ? `Poids total pour ces ${groupSize} colis` : locale === "nl" ? `Totaalgewicht voor deze ${groupSize} pakketten` : `Total weight for these ${groupSize} parcels`) : (locale === "fr" ? "Poids du colis" : locale === "nl" ? "Gewicht zending" : "Parcel weight")} <span className="optional-badge">{t.optional}</span></span><input type="number" min="0.001" max="100000" step="0.001" inputMode="decimal" placeholder="kg" value={parcel.weightKg} onChange={(event) => { const value = event.target.value; setParcelDrafts((rows) => rows.map((row) => row.key === parcel.key ? { ...row, weightKg: value } : row)); }} /><small>{groupSize > 1 ? (locale === "fr" ? `Réparti à égalité entre les ${groupSize} colis${effectiveWeightKg ? ` (= ${effectiveWeightKg} kg chacun)` : ""}.` : locale === "nl" ? `Gelijk verdeeld over de ${groupSize} pakketten${effectiveWeightKg ? ` (= ${effectiveWeightKg} kg elk)` : ""}.` : `Split evenly across the ${groupSize} parcels${effectiveWeightKg ? ` (= ${effectiveWeightKg} kg each)` : ""}.`) : (locale === "fr" ? "Laissez vide pour un objet volumineux (machine à laver, télé…)" : locale === "nl" ? "Laat leeg voor een groot voorwerp (wasmachine, tv…)" : "Leave blank for a bulky item (washing machine, TV…)")}</small></label> : <label><span className="field-label">{locale === "fr" ? "Poids du colis" : locale === "nl" ? "Gewicht zending" : "Parcel weight"}</span><div className="grouped-weight-note">{locale === "fr" ? `= ${effectiveWeightKg || "…"} kg (inclus avec le colis précédent)` : locale === "nl" ? `= ${effectiveWeightKg || "…"} kg (inbegrepen bij het vorige pakket)` : `= ${effectiveWeightKg || "…"} kg (included with the previous parcel)`}</div></label>}<label>{locale === "fr" ? "Prix" : locale === "nl" ? "Prijs" : "Price"}<input type="number" min="0.01" max="1000000" step="0.01" inputMode="decimal" placeholder={effectiveWeightKg ? (preview.priceAmount != null ? `${preview.priceAmount} ${preview.priceCurrency}` : (locale === "fr" ? "Renseignez le poids" : locale === "nl" ? "Vul het gewicht in" : "Enter the weight")) : (creationOriginCountry === "MA" ? "MAD" : "EUR")} value={parcel.manualPriceAmount} onChange={(event) => { const value = event.target.value; setParcelDrafts((rows) => rows.map((row) => row.key === parcel.key ? { ...row, manualPriceAmount: value } : row)); }} /><small>{effectiveWeightKg ? (locale === "fr" ? `Calculé à partir du poids (${creationOriginCountry === "MA" ? "15 DH/kg" : "1,50 €/kg"}) -- laissez vide pour le garder, ou indiquez un prix pour le remplacer.` : locale === "nl" ? `Berekend op basis van het gewicht (${creationOriginCountry === "MA" ? "15 DH/kg" : "1,50 €/kg"}) -- laat leeg om te behouden, of geef een prijs op om te overschrijven.` : `Calculated from weight (${creationOriginCountry === "MA" ? "15 MAD/kg" : "1.50 EUR/kg"}) -- leave blank to keep it, or enter a price to override.`) : (locale === "fr" ? "Objet volumineux : indiquez le prix directement" : locale === "nl" ? "Groot voorwerp: geef de prijs rechtstreeks op" : "Bulky item: enter the price directly")}</small></label><div className="payment-status-field"><span className="field-label">{locale === "fr" ? "Paiement" : locale === "nl" ? "Betaling" : "Payment"}</span><div className="payment-status-pills">{(["unpaid", "partial", "paid"] as const).map((status) => <button key={status} type="button" className={`payment-status-pill payment-status-${status}${parcel.paymentStatus === status ? " active" : ""}`} aria-pressed={parcel.paymentStatus === status} onClick={() => setParcelDrafts((rows) => rows.map((row) => row.key === parcel.key ? { ...row, paymentStatus: status, amountPaid: status === "partial" ? row.amountPaid : "" } : row))}>{status === "unpaid" ? (locale === "fr" ? "Impayé" : locale === "nl" ? "Onbetaald" : "Unpaid") : status === "partial" ? (locale === "fr" ? "Partiel" : locale === "nl" ? "Gedeeltelijk" : "Partial") : (locale === "fr" ? "Payé" : locale === "nl" ? "Betaald" : "Paid")}</button>)}</div>{parcel.paymentStatus === "partial" && <label><span className="field-label">{locale === "fr" ? "Montant déjà payé" : locale === "nl" ? "Reeds betaald bedrag" : "Amount paid so far"}</span><input type="number" min="0.01" max="1000000" step="0.01" inputMode="decimal" placeholder={creationOriginCountry === "MA" ? "MAD" : "EUR"} value={parcel.amountPaid} onChange={(event) => { const value = event.target.value; setParcelDrafts((rows) => rows.map((row) => row.key === parcel.key ? { ...row, amountPaid: value } : row)); }} /><small>{(() => { const remaining = effectivePriceAmount != null && Number(parcel.amountPaid) > 0 ? effectivePriceAmount - Number(parcel.amountPaid) : null; const effectiveCurrency = preview.priceCurrency ?? (creationOriginCountry === "MA" ? "MAD" : "EUR"); return remaining != null && Number.isFinite(remaining) ? (locale === "fr" ? `Reste dû : ${remaining.toLocaleString("fr-BE", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${effectiveCurrency}` : locale === "nl" ? `Nog verschuldigd: ${remaining.toLocaleString("nl-BE", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${effectiveCurrency}` : `Remaining: ${remaining.toLocaleString("en-GB", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${effectiveCurrency}`) : (locale === "fr" ? "Le reste dû sera calculé à partir du prix ci-dessus." : locale === "nl" ? "Het resterende bedrag wordt berekend op basis van de prijs hierboven." : "The remaining balance is computed from the price above."); })()}</small></label>}</div>{isAnchor && !effectiveWeightKg && <label>{locale === "fr" ? "Description de l'objet" : locale === "nl" ? "Omschrijving van het voorwerp" : "Item description"}<input type="text" required maxLength={200} placeholder={locale === "fr" ? "Ex. : télévision, lave-linge…" : locale === "nl" ? "Bijv. televisie, wasmachine…" : "E.g. TV, washing machine…"} value={parcel.itemDescription} onChange={(event) => { const value = event.target.value; setParcelDrafts((rows) => rows.map((row) => row.key === parcel.key ? { ...row, itemDescription: value } : row)); }} /><small>{locale === "fr" ? "Obligatoire pour un objet non pesé, pour le distinguer des autres colis." : locale === "nl" ? "Verplicht voor een niet-gewogen voorwerp, om het van andere zendingen te onderscheiden." : "Required for an unweighed item, to tell it apart from other parcels."}</small></label>}{parcelDrafts.length > 1 && <button type="button" className="remove-parcel-row" aria-label={locale === "fr" ? "Retirer ce colis" : locale === "nl" ? "Dit pakket verwijderen" : "Remove this parcel"} onClick={() => setParcelDrafts((rows) => rows.filter((row) => row.key !== parcel.key).map((row) => ({ ...row, groupedWithPrevious: false })))}>×</button>}</div>; })}{!editingDeliveryId && <button type="button" className="add-parcel-row" onClick={() => setParcelDrafts((rows) => [...rows, emptyParcelDraft(crypto.randomUUID())])}>{locale === "fr" ? "+ Ajouter un colis pour ce client" : locale === "nl" ? "+ Pakket toevoegen voor deze klant" : "+ Add another parcel for this customer"}</button>}</div><div className="form-row"><label><span className="field-label">{locale === "fr" ? "Date de départ" : locale === "nl" ? "Vertrekdatum" : "Departure date"} <span className="optional-badge">{t.optional}</span></span><input type="datetime-local" name="nextTruckDepartureAt" value={creationDepartureAt} onChange={(event) => setCreationDepartureAt(event.target.value)} /></label><label>{locale === "fr" ? "Date d'arrivée estimée" : locale === "nl" ? "Geschatte aankomstdatum" : "Estimated arrival date"}<div className="price-preview">{creationEstimatedArrival ? <strong>{creationEstimatedArrival.toLocaleString(locale === "fr" ? "fr-BE" : locale === "nl" ? "nl-BE" : "en-GB", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })}</strong> : <span>{locale === "fr" ? "Choisissez l'agence et la date de départ" : locale === "nl" ? "Kies het agentschap en de vertrekdatum" : "Choose the agency and departure date"}</span>}</div><small>{locale === "fr" ? "Calculée automatiquement selon le délai CTM de l'agence de destination." : locale === "nl" ? "Automatisch berekend volgens de CTM-termijn van het bestemmingsagentschap." : "Calculated automatically from the destination agency's CTM transit time."}</small></label></div>{company?.role === "dispatcher" && integration.connected && integration.vehicles.length > 0 && <div className="form-row"><label><span className="field-label">{locale === "fr" ? "Camion" : locale === "nl" ? "Vrachtwagen" : "Truck"}</span><select value={creationVehicleId} onChange={(event) => setCreationVehicleId(event.target.value)}><option value="">{locale === "fr" ? "À affecter plus tard" : locale === "nl" ? "Later toewijzen" : "Assign later"}</option>{integration.vehicles.map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{truckNumberLabel(vehicle.id) ? `${truckNumberLabel(vehicle.id)} · ${vehicle.name}` : vehicle.name}</option>)}</select>{editingDeliveryId && creationVehicleId && vehicleAssignmentConflict(creationVehicleId, editingDeliveryId) ? <small className="warning">{locale === "fr" ? `⚠ Ce camion est encore en route (${vehicleAssignmentConflict(creationVehicleId, editingDeliveryId)!.id}). Vérifiez qu'il sera bien de retour.` : locale === "nl" ? `⚠ Dit voertuig is nog onderweg (${vehicleAssignmentConflict(creationVehicleId, editingDeliveryId)!.id}). Controleer of het op tijd terug is.` : `⚠ This truck is still en route (${vehicleAssignmentConflict(creationVehicleId, editingDeliveryId)!.id}). Confirm it will actually be back.`}</small> : <small>{locale === "fr" ? "Le camion ne comptera dans le suivi qu'une fois le colis scanné « chargé »." : locale === "nl" ? "Het voertuig telt pas mee voor tracking zodra het pakket als \"geladen\" gescand is." : "The truck won't count toward tracking until the parcel is scanned as loaded."}</small>}</label></div>}{!editingDeliveryId && features.whatsappAvailable && <label className="consent-choice"><input type="checkbox" name="whatsappOptIn" defaultChecked={creationDraftSeed?.whatsappOptIn ?? false} /><span>{locale === "fr" ? "Nouveau consentement WhatsApp confirmé pour les numéros renseignés" : locale === "nl" ? "Nieuwe WhatsApp-toestemming bevestigd voor de ingevulde nummers" : "New WhatsApp consent confirmed for the entered numbers"}<small>{locale === "fr" ? "Inutile de cocher si ce numéro a déjà consenti auparavant : TrackFleet le reconnaît automatiquement. Le consentement peut toujours être retiré." : locale === "nl" ? "Niet nodig als dit nummer eerder toestemming gaf: TrackFleet herkent dit automatisch. Toestemming kan altijd worden ingetrokken." : "Do not check this when the number already consented: TrackFleet remembers it automatically. Consent can always be withdrawn."}</small></span></label>}<div className="modal-footer"><button type="button" onClick={closeCreateModal}>{t.cancel}</button><button className="primary-button" type="submit" disabled={creating}>{creating ? t.creating : editingDeliveryId ? (locale === "fr" ? "Enregistrer les modifications" : locale === "nl" ? "Wijzigingen opslaan" : "Save changes") : t.createDelivery}<span>→</span></button></div></form></section></div>}
