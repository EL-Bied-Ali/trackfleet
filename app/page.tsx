    ...
    return Array.from(groups.values())
      .map((group) => {
        ...
        const uniformDestination = group.deliveries.length > 1
          ...
          ? group.deliveries[0]
          : null;
        // Same idea as uniformDestination, but for the Agence column: a
        // truck run departs from exactly one place, so its origin is
        // uniform across every parcel in the group far more often than the
        // destination is -- repeating the same site label on every row
        // added real width to a column with nothing new to say. Not
        // assumed outright though (a group could in principle mix origins),
        // so it's still verified the same way.
