...
export default function Home() {
  ...
  const [creationVehicleId, setCreationVehicleId] = useState(UNASSIGNED_VEHICLE_ID);
  ...
  // The "next truck departure" date is a plain manually-typed field with no
  // check against reality -- a dispatcher could schedule a truck's next
  // departure while that same truck is still out on its current delivery.
  // Warns (doesn't block -- the dispatcher may know the truck is already
  // back even if its delivery isn't marked Delivered yet) when the selected
  // truck has another active delivery still in progress.
  const creationVehicleConflict = creationVehicleId !== UNASSIGNED_VEHICLE_ID
    ? deliveries.find((delivery) => delivery.sendatrackVehicleId === creationVehicleId && delivery.status !== "Delivered")
    : undefined;
  ...
  async function createDelivery(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (failure) {
      ...
      setCreationVehicleId(UNASSIGNED_VEHICLE_ID);
