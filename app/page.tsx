...
export default function Home() {
  ...
  async function createDelivery(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    // Truck assignment no longer happens at creation -- every new parcel
    // starts unassigned and gets a truck from the delivery table instead
    // (see the group-header truck editor), which avoids re-picking the same
    // truck for every unrelated parcel going out on the same run.
          ...
          <div className="map fleet-map">
            ...
            {!agencyMapUnavailable && showPopover && (selectedVehicle || deliveries.length > 0) && <div className="truck-popover">
              {selectedVehicle ? <>
                ...
                <dl><div><dt>{locale === "fr" ? "Vitesse" : locale === "nl" ? "Snelheid" : "Speed"}</dt><dd>{selectedVehicle.speed} km/h</dd></div></dl>
              </> : <>
