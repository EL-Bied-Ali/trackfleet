...
export default function Home() {
  ...
  const [scheduleEditorPlannedArrival, setScheduleEditorPlannedArrival] = useState("");
  ...
  function openCreateModal() {
    if (company) {
      const saved = window.localStorage.getItem(truckPreferenceKey(company));
      setCreationVehicleId(resolvePreferredTruck(saved, integration.vehicles.map((vehicle) => vehicle.id)));
    } else {
      setCreationVehicleId("");
    }
    setModalOpen(true);
  }
  ...
  async function createDelivery(event: React.FormEvent<HTMLFormElement>) {
    ...
    // Planned arrival and next-truck-departure both moved out of creation
    // (too redundant re-entering them per parcel) in favor of the same
    // table editor used for the truck itself -- every new delivery starts
    // with both unset.
