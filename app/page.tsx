        ...
        {visibleDeliveries.length === 0 ? <div className="deliveries-empty">
          ...
          {groupedDeliveries.map((group) => <tbody key={group.label}>
            <tr className="group-header-row"><td colSpan={company?.role === "dispatcher" ? 3 : 2}><div className="group-header-row-inner">{group.numberLabel && <span className="truck-number-badge" style={{ background: truckBadgeColor(group.truckNumber) }}>{group.numberLabel}</span>}<strong>{group.label}</strong><span>{group.deliveries.length} {locale === "fr" ? "colis" : locale === "nl" ? "pakketten" : group.deliveries.length === 1 ? "parcel" : "parcels"}</span></div></td><td className="col-journey">{group.uniformDestination && <>
              ...
              <span className="group-header-destination">{group.uniformDestination.destination}{staticKnownSite(group.uniformDestination.destinationSiteId)?.finalLegTrackingUnavailable && <b className="relay-badge">{locale === "fr" ? "Relais CTM" : locale === "nl" ? "CTM-relais" : "CTM relay"}</b>}</span>
