...
export const store: DeliveryStore = {
  ...
  // D1 has no RETURNING support in this codebase's usage (see
  // login-rate-limit.cloudflare.ts for the same two-step shape) -- write
  // the increment, then read the row back. Safe under concurrency because
  // D1/SQLite serializes writes per database.
  async assignShortCode(companyId, prefix) {
    await db().prepare(`INSERT INTO delivery_code_counters (company_id, prefix, next_number)
      VALUES (?, ?, 0)
      ON CONFLICT(company_id, prefix) DO UPDATE SET next_number = next_number + 1`)
      .bind(companyId, prefix).run();
    const row = await db().prepare("SELECT next_number FROM delivery_code_counters WHERE company_id = ? AND prefix = ? LIMIT 1")
      .bind(companyId, prefix).first<{ next_number: number }>();
    return `${prefix} ${String(row?.next_number ?? 0).padStart(2, "0")}`;
  },
