...
function deliveryStatement(db: D1Binding, delivery: DeliveryRow) {
  ...
  ON CONFLICT(id) DO UPDATE SET
    ...
    contact = excluded.contact, recipient_name = excluded.recipient_name, recipient_contact = excluded.recipient_contact, weight_kg = excluded.weight_kg, price_amount = excluded.price_amount, price_currency = excluded.price_currency, item_description = excluded.item_description, customer_email = excluded.customer_email,
    ...
    company_id = excluded.company_id, tracking_token = excluded.tracking_token, trip_id = excluded.trip_id, shipment_id = excluded.shipment_id, parcel_code = excluded.parcel_code, short_code = excluded.short_code, payment_status = excluded.payment_status, amount_paid = excluded.amount_paid`)
    .bind(delivery.id, delivery.customer, delivery.originSiteId, delivery.originLatitude, delivery.originLongitude,
      ...
      delivery.tripId ?? null, delivery.shipmentId ?? null, delivery.createdAt.getTime(), delivery.parcelCode ?? null, delivery.shortCode ?? null, delivery.paymentStatus ?? null, delivery.amountPaid ?? null);
}
