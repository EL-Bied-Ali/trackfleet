...
function deliveryStatement(db: D1Binding, delivery: DeliveryRow) {
  ...
  ON CONFLICT(id) DO UPDATE SET
    ...
    eta = excluded.eta, planned_arrival_at = excluded.planned_arrival_at, progress = excluded.progress, color = excluded.color,
    contact = excluded.contact, weight_kg = excluded.weight_kg, price_amount = excluded.price_amount, price_currency = excluded.price_currency,
    whatsapp_opt_in = excluded.whatsapp_opt_in, whatsapp_opt_in_at = excluded.whatsapp_opt_in_at,
    ...
    .bind(delivery.id, delivery.customer, delivery.originSiteId, delivery.originLatitude, delivery.originLongitude,
      ...
      delivery.plannedArrivalAt?.getTime() ?? null, delivery.progress, delivery.color, delivery.contact,
      delivery.weightKg ?? null, delivery.priceAmount ?? null, delivery.priceCurrency ?? null,
      delivery.whatsappOptIn === true ? 1 : 0, delivery.whatsappOptInAt?.getTime() ?? null,
