...
function eventStatement(db: D1Binding, event: DeliveryEventRow) {
  return db.prepare(`INSERT INTO delivery_events (delivery_id, type, progress, created_at)
    VALUES (?, ?, ?, ?)
    ON CONFLICT(delivery_id, type) DO UPDATE SET
      progress = excluded.progress,
...
function etaStatement(db: D1Binding, observation: EtaObservationRow) {
  ...
  ON CONFLICT(delivery_id, position_at) DO UPDATE SET
    ...
    .bind(
      ...
      observation.positionAt.getTime(),
      observation.estimatedArrivalAt.getTime(),
      observation.plannedArrivalAt?.getTime() ?? null,
      observation.delayMinutes,
      observation.effectiveSpeedKmh,
      observation.remainingDistanceKm,
...
function siteStatement(db: D1Binding, site: CompanySite) {
  return db.prepare(`INSERT INTO sites (
    company_id, id, label, city, country, address, latitude, longitude, arrival_radius_km, roles, created_at, updated_at
  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
function tripStatement(db: D1Binding, trip: TripRecord) {
  ...
  return db.prepare(`INSERT INTO trips (
...
function sessionStatement(db: D1Binding, session: SessionRow) {
  return db.prepare(`INSERT INTO sessions (
  ...
  ON CONFLICT(token_hash) DO UPDATE SET
    ...
    .bind(
      ...
      session.user_label,
      session.credentials_ciphertext,
...
export async function reconcileD1Standby(): Promise<D1ReconciliationResult> {
  const db = d1();
  if (!db) {
    return { ran: false, reason: "d1_not_bound", companies: 0, deliveries: 0, events: 0, etaObservations: 0, sites: 0, trips: 0, sessions: 0 };
  }
  const url = databaseUrl();
  if (!url) {
    return { ran: false, reason: "postgres_not_configured", companies: 0, deliveries: 0, events: 0, etaObservations: 0, sites: 0, trips: 0, sessions: 0 };
  }
  ...
  const sql = neon(url);
  try {
    const companyRows = await sql`SELECT company_id AS id
      FROM deliveries
      WHERE company_id IS NOT NULL AND company_id <> ''
      GROUP BY company_id
      ORDER BY MAX(created_at) DESC
      LIMIT ${maxCompanies}` as Array<{ id: string }>;
    ...
    const sessionRows = await sql`SELECT token_hash, company_id, account_label, user_label, credentials_ciphertext, expires_at, created_at
      ...
      LIMIT ${maxActiveSessions}` as SessionRow[];
    ...
    for (const companyId of companyIds) {
      const [deliveries, sites, trips] = await Promise.all([
        store.listForCompany(companyId),
        siteStore.listForCompany(companyId),
        store.listTrips(companyId, maxTripsPerCompany),
      ]);
      ...
      statements.push(...sites.map((site) => siteStatement(db, site)));
      statements.push(...trips.map((trip) => tripStatement(db, trip)));
      for (const events of eventGroups) {
        eventsCount += events.length;
        statements.push(...events.map((event) => eventStatement(db, event)));
      }
      for (const observations of etaGroups) {
        etaCount += observations.length;
        statements.push(...observations.map((observation) => etaStatement(db, observation)));
      }
    }
