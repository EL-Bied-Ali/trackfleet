...
function deliveryStatement(db: D1Binding, delivery: DeliveryRow) {
  ...
  ON CONFLICT(id) DO UPDATE SET
    ...
    company_id = excluded.company_id, tracking_token = excluded.tracking_token, trip_id = excluded.trip_id, shipment_id = excluded.shipment_id, parcel_code = excluded.parcel_code, short_code = excluded.short_code`)
    .bind(delivery.id, delivery.customer, delivery.originSiteId, delivery.originLatitude, delivery.originLongitude,
      ...
      delivery.tripId ?? null, delivery.shipmentId ?? null, delivery.createdAt.getTime(), delivery.parcelCode ?? null, delivery.shortCode ?? null);
}
