...
async function seed(companyId: string) {
  ...
  for (const site of knownSites) {
    await db().prepare(`INSERT OR IGNORE INTO sites (company_id,id,label,city,country,address,latitude,longitude,arrival_radius_km,roles,whatsapp,color,short_code_prefix,created_at,updated_at)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(companyId, site.id, site.label, site.city, site.country, site.address, site.latitude, site.longitude, site.arrivalRadiusKm, JSON.stringify(site.roles), site.whatsapp ?? null, site.color ?? null, site.shortCodePrefix ?? null, now, now).run();
  }
}
...
export const siteStore: SiteStore = {
  ...
  async upsert(input: CreateCompanySiteInput) {
    const now = Date.now();
    await db().prepare(`INSERT INTO sites (company_id,id,label,city,country,address,latitude,longitude,arrival_radius_km,roles,whatsapp,color,short_code_prefix,created_at,updated_at)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
      ON CONFLICT(company_id,id) DO UPDATE SET label=excluded.label,city=excluded.city,country=excluded.country,address=excluded.address,latitude=excluded.latitude,longitude=excluded.longitude,arrival_radius_km=excluded.arrival_radius_km,roles=excluded.roles,whatsapp=excluded.whatsapp,color=excluded.color,short_code_prefix=excluded.short_code_prefix,updated_at=excluded.updated_at`)
      .bind(input.companyId,input.id,input.label,input.city,input.country,input.address,input.latitude,input.longitude,input.arrivalRadiusKm,JSON.stringify(input.roles),input.whatsapp ?? null,input.color ?? null,input.shortCodePrefix ?? null,now,now).run();
