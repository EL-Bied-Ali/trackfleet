...
async function seed(companyId: string) {
  ...
  await sql`INSERT INTO sites (company_id,id,label,city,country,address,latitude,longitude,arrival_radius_km,roles,whatsapp,color)
    SELECT ${companyId}, seed.id, seed.label, seed.city, seed.country, seed.address,
      seed.latitude, seed.longitude, seed.arrival_radius_km, seed.roles, seed.whatsapp, seed.color
...
export const postgresSiteStore: SiteStore = {
  ...
  async upsert(input: CreateCompanySiteInput) {
    const rows = await sql`INSERT INTO sites (company_id,id,label,city,country,address,latitude,longitude,arrival_radius_km,roles,whatsapp,color,updated_at)
      VALUES (${input.companyId},${input.id},${input.label},${input.city},${input.country},${input.address},${input.latitude},${input.longitude},${input.arrivalRadiusKm},${JSON.stringify(input.roles)},${input.whatsapp ?? null},${input.color ?? null},now())
      ON CONFLICT (company_id,id) DO UPDATE SET label=excluded.label,city=excluded.city,country=excluded.country,address=excluded.address,latitude=excluded.latitude,longitude=excluded.longitude,arrival_radius_km=excluded.arrival_radius_km,roles=excluded.roles,whatsapp=excluded.whatsapp,color=excluded.color,updated_at=now()
      RETURNING *`;
